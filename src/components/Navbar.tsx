import React, { useState } from 'react';
import { FiMenu, FiX } from "react-icons/fi";
import ContactModal from './ui/ContactModal';
import { IoIosArrowDown } from "react-icons/io";
import { motion, AnimatePresence } from "framer-motion";

export const Navbar: React.FC = () => {
  const [menuOpen, setMenuOpen] = useState(false);
  const [openModal, setOpenModal] = useState(false);
  const [open, setOpen] = useState(false);

  const handleModal = () => setOpenModal(true);

  const navigationItems = ["Home", "About", "Gallery", "Contact"];

  return (
    <header className="fixed w-full top-6 left-0 z-50 flex justify-center"
      data-aos="fade-down"
      data-aos-easing="linear"
      data-aos-duration="1000"
    >
      {/* Desktop Navigation */}
      <div className="hidden md:flex items-center gap-8 px-8 py-3 rounded-full 
        backdrop-blur-md bg-white/10 border border-white/20 shadow-lg">
        
        {/* Left Menu */}
        <nav className="flex items-center gap-6 text-white/70 text-lg">
          <span className="cursor-pointer hover:text-white transition">Home</span>
          <span className="text-white/90">•</span>
          <span className="cursor-pointer hover:text-white transition"
            onClick={() => document.getElementById("about")?.scrollIntoView({ behavior: "smooth" })}>
            About
          </span>
        </nav>

        {/* Center Logo */}
        <div className="w-[100px] h-[35px] rounded-xl flex items-center justify-center">
          <img src="/q.png" alt="Center Icon" className="w-[70px] h-[70px]" />
        </div>

        {/* Right Menu */}
        <nav className="flex items-center gap-6 text-white/70 text-lg">
          <span className="cursor-pointer hover:text-white transition" onClick={() => window.open("https://emarq-gallery.vercel.app", "_blank")}>Gallery</span>
          <span className="text-white/90">•</span>
          <span className="cursor-pointer hover:text-white transition" onClick={handleModal}>Contact</span>
        </nav>
      </div>

      <ContactModal isOpen={openModal} onClose={() => setOpenModal(false)} />

      {/* Mobile Navigation */}
      <div className="md:hidden w-full flex justify-between items-center px-3 py-3 absolute top-0 z-50">
        {/* Logo */}
        <div>
          <img src="/q.png" alt="Logo" className="w-[50px] h-[50px]" />
        </div>

        {/* Hamburger / Close */}
        <div>
          {menuOpen ? (
            <FiX
              className="text-white text-2xl cursor-pointer"
              onClick={() => setMenuOpen(false)}
            />
          ) : (
            <FiMenu
              className="text-white text-2xl cursor-pointer"
              onClick={() => setMenuOpen(true)}
            />
          )}
        </div>
      </div>

      {/* Mobile Menu Overlay */}
      <AnimatePresence>
  {menuOpen && (
    <motion.div
      key="mobileMenu"
      initial={{ y: -200, opacity: 0 }}
      animate={{ y: 0, opacity: 1 }}
      exit={{ y: -200, opacity: 0 }}
      transition={{ type: "spring", stiffness: 100, damping: 20 }}
      className="fixed -top-[27px] left-0 w-full flex flex-col items-center justify-center gap-6 p-8 text-white backdrop-blur-md bg-white/10 border border-white/20 shadow-lg md:hidden"
    >
      {navigationItems.map((item, index) => (
        <motion.span
          key={item}
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          exit={{ y: 20, opacity: 0 }}
          transition={{ delay: index * 0.08 }}
          className="cursor-pointer hover:text-gray-300 text-lg font-medium"
          onClick={() => {
            setMenuOpen(false); // close mobile menu

            if (item === "Home") {
              document.getElementById("home")?.scrollIntoView({ behavior: "smooth" });
            } else if (item === "About") {
              document.getElementById("about")?.scrollIntoView({ behavior: "smooth" });
            } else if (item === "Gallery") {
              window.open("https://emarq-gallery.vercel.app/", "_blank");
            } else if (item === "Contact") {
              setOpenModal(true);
            }
          }}
        >
          {item.split("").map((letter, i) => (
            <motion.span
              key={i}
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
              transition={{ delay: i * 0.03 + index * 0.08 }}
            >
              {letter}
            </motion.span>
          ))}
        </motion.span>
      ))}
    </motion.div>
  )}
</AnimatePresence>
<ContactModal isOpen={openModal} onClose={() => setOpenModal(false)} />

      {/* Arrow + SVG div - only show on large screens */}
      <div className="hidden md:block fixed top-2 right-8 z-[10000]">
        <motion.div
          onClick={() => setOpen(!open)}
          initial={{ height: 48 }}
          animate={{ height: open ? 150 : 48 }}
          transition={{ type: "spring", stiffness: 200, damping: 25 }}
          className="w-12 relative flex flex-col items-center justify-start rounded-full cursor-pointer  backdrop-blur-md bg-white/10 border border-white/20 shadow-lg overflow-hidden"
        >
          {/* Arrow at the top */}
          <motion.div
            className="flex items-center justify-center w-12 h-12"
            animate={{ rotate: open ? 180 : 0 }}
            transition={{ duration: 0.3 }}
          >
            <IoIosArrowDown style={{ color: "#E8C1C5", fontSize: 32 }} />
          </motion.div>

          {/* Dropdown SVGs */}
          <AnimatePresence>
            {open && (
              <motion.div
                key="svg"
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 5 }}
                exit={{ opacity: 0, y: -10 }}
                transition={{ type: "spring", stiffness: 200, damping: 20 }}
                className="flex flex-col items-center justify-center w-[40px] gap-0.5 pb-[35px]"
              >
                {/* Gmail and WhatsApp SVGs */}
                <a href="mailto:Info@emarq.ae" className="inline-block">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 141.7 141.7" className="w-16 h-16" > <path fill="#4285f4" d="M24.3,111.3h17.2V69.6L16.9,51.3V104C16.9,108,20.2,111.3,24.3,111.3z" /> <path fill="#34a853" d="M100.3,111.3h17.2c4.1,0,7.4-3.3,7.4-7.4V51.3l-24.5,18.4V111.3z" /> <path fill="#fbbc04" d="M100.3,37.8v31.9l24.5-18.4v-9.8c0-9.1-10.4-14.3-17.7-8.8L100.3,37.8z" /> <path fill="#ea4335" fillRule="evenodd" d="M41.4,69.6V37.8l29.4,22.1l29.4-22.1v31.9L70.9,91.7L41.4,69.6z" clipRule="evenodd" /> <path fill="#c5221f" d="M16.9,41.4v9.8l24.5,18.4V37.8l-6.9-5.2C27.3,27.2,16.9,32.4,16.9,41.4z" /> </svg>
                  {/* Gmail SVG code */}
                </a>
                <a href="https://wa.me/971506568512" target='_blank' rel='noopener noreferrer'>
                  {/* WhatsApp SVG code */}
                  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 1219.547 1225.016" id="whatsapp"> <path fill="#E0E0E0" d="M1041.858 178.02C927.206 63.289 774.753.07 612.325 0 277.617 0 5.232 272.298 5.098 606.991c-.039 106.986 27.915 211.42 81.048 303.476L0 1225.016l321.898-84.406c88.689 48.368 188.547 73.855 290.166 73.896h.258.003c334.654 0 607.08-272.346 607.222-607.023.056-162.208-63.052-314.724-177.689-429.463zm-429.533 933.963h-.197c-90.578-.048-179.402-24.366-256.878-70.339l-18.438-10.93-191.021 50.083 51-186.176-12.013-19.087c-50.525-80.336-77.198-173.175-77.16-268.504.111-278.186 226.507-504.503 504.898-504.503 134.812.056 261.519 52.604 356.814 147.965 95.289 95.36 147.728 222.128 147.688 356.948-.118 278.195-226.522 504.543-504.693 504.543z"></path> <linearGradient id="a" x1="609.77" x2="609.77" y1="1190.114" y2="21.084" gradientUnits="userSpaceOnUse"> <stop offset="0" stop-color="#20b038"></stop> <stop offset="1" stop-color="#60d66a"></stop> </linearGradient> <path fill="url(#a)" d="M27.875 1190.114l82.211-300.18c-50.719-87.852-77.391-187.523-77.359-289.602.133-319.398 260.078-579.25 579.469-579.25 155.016.07 300.508 60.398 409.898 169.891 109.414 109.492 169.633 255.031 169.57 409.812-.133 319.406-260.094 579.281-579.445 579.281-.023 0 .016 0 0 0h-.258c-96.977-.031-192.266-24.375-276.898-70.5l-307.188 80.548z"></path> <path fill="#FFF" fill-rule="evenodd" d="M462.273 349.294c-11.234-24.977-23.062-25.477-33.75-25.914-8.742-.375-18.75-.352-28.742-.352-10 0-26.25 3.758-39.992 18.766-13.75 15.008-52.5 51.289-52.5 125.078 0 73.797 53.75 145.102 61.242 155.117 7.5 10 103.758 166.266 256.203 226.383 126.695 49.961 152.477 40.023 179.977 37.523s88.734-36.273 101.234-71.297c12.5-35.016 12.5-65.031 8.75-71.305-3.75-6.25-13.75-10-28.75-17.5s-88.734-43.789-102.484-48.789-23.75-7.5-33.75 7.516c-10 15-38.727 48.773-47.477 58.773-8.75 10.023-17.5 11.273-32.5 3.773-15-7.523-63.305-23.344-120.609-74.438-44.586-39.75-74.688-88.844-83.438-103.859-8.75-15-.938-23.125 6.586-30.602 6.734-6.719 15-17.508 22.5-26.266 7.484-8.758 9.984-15.008 14.984-25.008 5-10.016 2.5-18.773-1.25-26.273s-32.898-81.67-46.234-111.326z" clip-rule="evenodd"></path> <path fill="#FFF" d="M1036.898 176.091C923.562 62.677 772.859.185 612.297.114 281.43.114 12.172 269.286 12.039 600.137 12 705.896 39.633 809.13 92.156 900.13L7 1211.067l318.203-83.438c87.672 47.812 186.383 73.008 286.836 73.047h.255.003c330.812 0 600.109-269.219 600.25-600.055.055-160.343-62.328-311.108-175.649-424.53zm-424.601 923.242h-.195c-89.539-.047-177.344-24.086-253.93-69.531l-18.227-10.805-188.828 49.508 50.414-184.039-11.875-18.867c-49.945-79.414-76.312-171.188-76.273-265.422.109-274.992 223.906-498.711 499.102-498.711 133.266.055 258.516 52 352.719 146.266 94.195 94.266 146.031 219.578 145.992 352.852-.118 274.999-223.923 498.749-498.899 498.749z"></path> </svg>
                  </a>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
      </div>
    </header>
  );
};
